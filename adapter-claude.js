...
function inspectElementState(el) {
  ...
  const inThinking = el.closest(
    '[data-testid="thinking-block"], .thinking-block, [data-is-thinking="true"], details'
  );
  if (inThinking) return 'thinking';

  const inResponse = el.closest(
    '[data-testid="chat-message-content"], .font-claude-message, .prose, .markdown, article, [data-is-streaming="true"], [role="region"]'
  );
...
function startAIObserver() {
  ...
  aiObserver = new MutationObserver((mutations) => {
    ...
    for (const m of mutations) {
      ...
      const stateType = inspectElementState(el);
...
function setupUserDetection() {
  ...
  setInterval(() => {
    ...
    if (generating) {
      ...
      setTimeout(() => {
        if (!isClaudeGenerating() && (currentState === STATES.AI_TYPING || currentState === STATES.AI_THINKING)) {
          setState(STATES.AI_COMPLETE);
          stopAIObserver();
          scheduleReset();
        }
